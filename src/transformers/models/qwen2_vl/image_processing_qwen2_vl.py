...
class Qwen2VLImageProcessor(BaseImageProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Handle both old and new size format for backward compatibility
        if size is not None:
            if "min_pixels" in size and "max_pixels" in size:
                # New format with min_pixels/max_pixels
                size = {"shortest_edge": size["min_pixels"], "longest_edge": size["max_pixels"]}
            elif "shortest_edge" not in size or "longest_edge" not in size:
                raise ValueError("size must contain either ('shortest_edge' and 'longest_edge') or ('min_pixels' and 'max_pixels') keys.")
        else:
