...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        # remove this since it will raise an error
        # expected_seq_length = self.config.max_source_positions * self.conv1.stride[0] * self.conv2.stride[0]
        # if input_features.shape[-1] != expected_seq_length:
        #     raise ValueError(
        #         f"Whisper expects the mel input features to be of length {expected_seq_length}, but found {input_features.shape[-1]}. Make sure to pad the input mel features to {expected_seq_length}."
        ...
        # add the dynamic pos embeddings
        pe_base = self.embed_positions.weight
        L_enc = inputs_embeds.size(1)
        L_base, d_model = pe_base.size()

        if L_enc <= L_base:
            embed_pos_dyn = pe_base[:L_enc, :]
        else:
            embed_pos_dyn = nn.functional.interpolate(
                    pe_base.T.unsqueeze(0),                                    
                    size=L_enc,
                    mode="linear",
                    align_corners=False,
                ).squeeze(0).T                                                 

        hidden_states = inputs_embeds + embed_pos_dyn.to(inputs_embeds.dtype).unsqueeze(0)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
